...
- (void)LoginCloud {
    ...
    CSCloudServer *oneDrive = [CSCloudServer cloudServerForType:CSOneDrive];
    if (oneDrive) {
        oneDrive.delegate = self;
        [chooseLoginCtr.serverArr addObject:oneDrive];
    }
    CSCloudServer *skyDrive = [CSCloudServer cloudServerForType:CSSkyDrive];
    if (skyDrive) {
