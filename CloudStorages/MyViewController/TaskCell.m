...
- (void)displayWithTask:(CSDownloadTask *)task{
    ...
    if (task.fileSize != 0) {
        self.prossLable.text = [NSString stringWithFormat:@" File size%0.0fkb  %0.0f%%", (float)task.fileSize/1000.0,((float)task.receivedSize/task.fileSize) * 100];
    }else{
...
- (void)taskDownloadProgress{
    if (self.cellTask.fileSize != 0) {
        self.prossLable.text = [NSString stringWithFormat: @" File size%0.0fkb  %0.0f%%", (float)self.cellTask.fileSize/1000.0,((float)self.cellTask.receivedSize/self.cellTask.fileSize) * 100];
    }else{
        self.prossLable.text = @"0";
    }
}
...
/**
 Callback of download task failure
